...
class OeuvreExposeeManager {
    ...
    public function ListOeuvresPrevues($idExpo) {
        $list = [];
        $q = $this->_db->query("SELECT O.idOeuvre, titre, longueur, hauteur, etat, image, qrcode, descriptifFR, idTypeOeuvre, idArtiste, idCollectif FROM Oeuvre O, OeuvreExposee E WHERE O.idOeuvre = E.idOeuvre AND dateEntree = '1970-01-01' AND idExpo ='".$idExpo."' ORDER BY titre");
        while ($data = $q->fetch()) {
            $list[] = new Oeuvre($data);
        }
