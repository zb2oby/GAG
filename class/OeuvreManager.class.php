...
class OeuvreManager {
    ...
    public function getSearch($saisie) {
        $list = [];
        $q = $this->_db->query("SELECT * FROM Oeuvre WHERE titre IS NULL OR titre LIKE '%".$saisie."%' OR longueur LIKE '%".$saisie."%' OR hauteur LIKE '%".$saisie."%' OR etat LIKE '%".$saisie."%' ORDER BY titre DESC");
        while ($data = $q->fetch()) {
            $list[] = $data;
        }
